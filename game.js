...
const TOWER_DEFS = {
  ...
  cryo: {
    ...
    dmg:11, range:110, rate:600, projSpeed:7, color:'#7af0ff', slow:0.5, slowDur:1500,
    desc:'Slows enemies on hit. Stacks reduce armor effectively.',
    upgradeCostMul:0.75, maxLevel:3
  },
  emp: {
    id:'emp', name:'EMP Tower', icon:'◎', cost:150,
    dmg:15, range:125, rate:1100, projSpeed:0, color:'#ffe14d', empBurst:true, empRadius:80,
  ...
  detector: {
    ...
    dmg:17, range:140, rate:700, projSpeed:8, color:'#0fffa0', revealStealth:true,
    desc:'Reveals and damages cloaked enemies others cannot hit.',
    upgradeCostMul:0.75, maxLevel:3
  },
  disruptor: {
    id:'disruptor', name:'Shield Disruptor', icon:'⬢', cost:135,
    dmg:9, range:130, rate:500, projSpeed:8, color:'#c46bff', shieldStrip:0.6, bonusVsArmored:1.2,
    desc:'Strips shields and shreds armor. Devastating vs protected foes.',
    upgradeCostMul:0.8, maxLevel:3
  }
};
...
class Projectile{
  constructor(opts){
    Object.assign(this, {
      ...
      shieldStrip:0, empBurst:false, empRadius:0, life:0, bonusVsArmored:0
    }, opts);
    this.id = nextId();
    this.dead = false;
  ...
  onHit(){
    ...
    if(this.pierce){
      ...
      if(this.target && this.target.alive){
        ...
        if(this.bonusVsArmored && (this.target.shieldMax>0 || this.target.armor>0)){
          finalDmg *= (1+this.bonusVsArmored);
        }
        dealDamageToEnemy(this.target, finalDmg);
      }
    }
  }
}
...
function dealDamageToEnemy(enemy, dmg){
  let finalDmg = dmg;
  let isCrit = false;
  if(Math.random() < state.mods.critChance){ finalDmg *= 2; isCrit = true; }
  spawnDamageNumber(enemy.x, enemy.y, Math.round(finalDmg), isCrit);
...
function onEnemyKilled(enemy, goldGained){
  ...
  if(enemy.isBoss) triggerShake(14);
  state.killStreak++;
  state.killStreakTimer = 1400; // ms window to keep the streak alive
}
...
class Tower{
  ...
  fire(){
    ...
    if(this.def.empBurst){
      // instant area burst — disables shield/armor and damages everyone in radius immediately
      const radius = this.def.empRadius;
      state.enemies.forEach(e=>{
        if(!e.alive) return;
      ...
      spawnShockwaveFX(this.x, this.y, radius, this.def.color);
      return;
    }
    const proj = new Projectile({
      ...
      bonusVsArmored:this.def.bonusVsArmored||0
    });
    if(this.def.slow){
...
function spawnDamageNumber(x,y,amount,isCrit){
  // skip tiny chip damage to avoid number spam clutter on fast-firing towers
  if(amount < 3 && !isCrit) return;
  state.particles.push({
    x:x+(Math.random()-0.5)*10, y:y-8, vx:(Math.random()-0.5)*20, vy:-46,
    life:0.65, maxLife:0.65, kind:'dmgnum', amount, isCrit
  });
}
function triggerShake(amount){
  state.shakeAmount = Math.max(state.shakeAmount, amount);
}
function triggerCoreFlash(){
  state.coreFlash = 1;
}
function updateParticles(dt){
  for(let i=state.particles.length-1;i>=0;i--){
    ...
    } else if(p.kind==='dmgnum'){
      p.x += p.vx*dt; p.y += p.vy*dt;
      p.vy += 60*dt; // gravity decel on the rise
    }
...
function gameUpdate(dt){
  ...
  updateParticles(dt);
  state.shakeAmount = Math.max(0, state.shakeAmount - dt*40);
  state.coreFlash = Math.max(0, state.coreFlash - dt*1.8);
...
function drawParticle(p){
  if(p.kind==='spark'){
    ...
    ctx.font = p.isCrit? '700 14px Orbitron, sans-serif' : '600 11px Chakra Petch, sans-serif';
    ctx.textAlign='center'; ctx.textBaseline='middle';
    ctx.fillStyle = p.isCrit? '#ffe14d' : '#ffffff';
    if(p.isCrit){ ctx.shadowColor='#ffe14d'; ctx.shadowBlur=8; }
    ctx.fillText((p.isCrit?'CRIT ':'')+Math.round(p.amount), p.x, p.y);
    ctx.shadowBlur=0;
    ctx.globalAlpha=1;
  }
}
...
function render(){
  ...
  if(state.coreFlash > 0.01){
    ctx.save();
    ctx.globalAlpha = Math.min(0.45, state.coreFlash*0.45);
...
function updateHUD(){
  ...
  if(state.killStreak >= 3){
    ...
    document.getElementById('stat-streak').textContent = 'x'+state.killStreak;
    streakChip.classList.toggle('hot', state.killStreak >= 8);
  } else {
...
function initGame(difficulty){
  ...
  state.techPurchased={};
  state.shakeAmount=0; state.coreFlash=0;
  state.killStreak=0; state.killStreakTimer=0;
  recalcMods();
